...
function drawExample() {
    for (let i = 0; i < tiles.length; i++) {
        for (let j = 0; j < tiles[0].length; j++) {
            ...
            let y = i * 8 + 20;
            if (tiles[i][j]) {
                CEctx.srcToDst(4, 12, 8, 8, x + 4, y + 4, PTctx, globalScale);
            } else {
                // no need to run for empty tiles
                continue;
            }
            if (!(top || tr || right)) CEctx.srcToDst(12, 8, 4, 4, x + 12, y, PTctx, globalScale);
            if (!right) CEctx.srcToDst(12, 12, 4, 8, x + 12, y + 4, PTctx, globalScale);
            if (!(right || br || bottom)) CEctx.srcToDst(12, 20, 4, 4, x + 12, y + 12, PTctx, globalScale);
            if (!bottom) CEctx.srcToDst(4, 20, 8, 4, x + 4, y + 12, PTctx, globalScale);
            if (!(bottom || bl || left)) CEctx.srcToDst(0, 20, 4, 4, x, y + 12, PTctx, globalScale);
            if (!left) CEctx.srcToDst(0, 12, 4, 8, x, y + 4, PTctx, globalScale);
            if (!(left || tl || top)) CEctx.srcToDst(0, 8, 4, 4, x, y, PTctx, globalScale);
            if (!top) CEctx.srcToDst(4, 8, 8, 4, x + 4, y, PTctx, globalScale);
        }
    }
    ...
    for (let i = 0; i < tiles.length; i++) {
        for (let j = 0; j < tiles[0].length; j++) {
            ...
            let x = j * 8 + 20;
            let y = i * 8 + 20;
            if (top && right && !tr) CEctx.srcToDst(0, 4, 4, 4, x + 12, y, PTctx, globalScale);
            if (right && bottom && !br) CEctx.srcToDst(0, 0, 4, 4, x + 12, y + 12, PTctx, globalScale);
            if (bottom && left && !bl) CEctx.srcToDst(4, 0, 4, 4, x, y + 12, PTctx, globalScale);
            if (left && top && !tl) CEctx.srcToDst(4, 4, 4, 4, x, y, PTctx, globalScale);
        }
    }
}
